...
CHANGELOG = [
    {
        "version": "v3.5",
        "date": "2026-04-20",
        "title": "Max-чекер, постинг в каналы, импорт контактов",
        "groups": [
            {"tag": "new", "items": [
                "<strong>Max-чекер</strong> — проверка наличия номера или User ID в MAX до рассылки. Три режима: Мягкая (безопасная, до 50-70/сут), Массовый чекинг (пачками по 100, внимание — перезаписывает имена в контактах), Чекер User ID (самый безопасный, без изменений).",
                "<strong>Планировщик постов</strong> — публикация в каналы/группы MAX по расписанию. Календарный вид с навигацией по месяцам, клик по дню создаёт пост, фоновый воркер проверяет просроченные посты раз в минуту.",
                "<strong>Импорт контактов из групп</strong> — dual-pane UI: слева все участники с поиском и пагинацией, справа выбранные, одним кликом импорт в базу лидов.",
                "<strong>Пред-валидация инвайтинга</strong> — в инвайтере появился сворачиваемый блок «Отсев невалидных номеров»: вставляете номера, чекер проверяет, их User ID автоматически добавляются в список для инвайта.",
            ]},
            {"tag": "improve", "items": [
                "<strong>Чекер на странице /app/checker</strong> полностью переделан — от формы-однострочника до полноценного инструмента с результатами в таблице и экспортом CSV.",
                "<strong>Endpoint /app/posts/account-chats/{id}</strong> возвращает только те чаты где аккаунт owner/admin — исключает попытки постить в чужие группы.",
                "<strong>Пост-воркер</strong> обёрнут в централизованный обработчик ошибок (как остальные 10 background-задач).",
            ]},
            {"tag": "fix", "items": [
